...
const GroupsSchedule = () => {
    ...
    const {data: academicSpecId} = useGetIdForAcademicSpecializationAbbreviationQuery(abbreviation);
    const [mapGroupCoursesToUser, { isLoading, isSuccess, data, error }] = useMapGroupCoursesToUserMutation()
    useEffect(() => {
        if (academicSpecId) {
    ...
    const {
        data: groupsData, isLoading: isGroupsLoading, error: groupsError,
    } = useGetAllGroupsWithAcademicSpecializationIdAndYearQuery({
        academicSpecializationId, year,
    });
    ...
    useEffect(() => {
        if (groupsList && groupsList.length > 0) {
            const fetchSchedules = async () => {
                try {
                    const schedules = await Promise.all(groupsList.map(async (group) => {
                    ...
                    setGroupSchedules([...schedules].sort((a, b) => a.group.localeCompare(b.group)));
                } catch (error) {
    ...
    const handleToggleAccordion = (group) => {
        setExpandedGroups((prevState) => ({
            ...prevState, [group]: !prevState[group],
        }))
    }

    const handleMarkAsMySchedule = async (group) => {
        console.log("userId", userId, "group", group);
        const response = await mapGroupCoursesToUser({userId: userId, group: group}).unwrap();
    };
        ...
        <div className="page-container-gt">
            ...
            {groupSchedules.length > 0 ? (<>
                {groupSchedules.map((schedule) => {
                    ...
                    return (<div key={schedule.group} className="accordion-item-gt">
                        <div
                            className="accordion-header-gt"
                            onClick={() => handleToggleAccordion(schedule.group)}
                        >
                            <h2>
                                {schedule.group === "Orarul tau" ? "Orarul tau" : `Grupa ${schedule.group}`}
                            </h2>
                            {isExpanded ?
                                <button
                                    className="mark-my-schedule-btn"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        handleMarkAsMySchedule(schedule.group);
                                    }}
                                >
                                    Selecteaza drept orarul tau
                                </button>
                                : null}
                        ...
                        {isExpanded && (<div className="accordion-content-gt">
