...
const GroupsScheduleTabelar = ({scheduleData, group, showHeader}) => {
    ...
    return (<div className="table-container-gt">
        {showHeader ? (
            <h2 className="table-title-gt">{group === "Orarul tau" ? "Orarul tau" : "Grupa " + group}</h2>) : null}
        <table className="table-gt">
            <thead>
            <tr>
                {isMobileView ? (<th>Ziua si Orele</th>) : (<>
                    <th>Ziua</th>
                    <th>Orele</th>
                </>)}
                <th>Frecventa</th>
            ...
            <tbody>
            {scheduleData.map((item, index) => (<tr key={index}>
                {isMobileView ? (<td className="mobile-view-td-gt">
                ...
                <td data-label="Frecventa">
                    {item.frequency === 0 ? "Săptămânal" : item.frequency === 1 ? "Săptămâna impară" : "Săptămâna pară"}
                </td>
                <td data-label="Sala">
                    <Link
